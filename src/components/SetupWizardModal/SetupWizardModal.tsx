...
export const SetupWizardModal: React.FC<SetupWizardModalProps> = ({
  ...
  const {
    ...
    prompt: wizardPrompt,
    config: autoConfig,
  } = useActiveWizardPrompt(tasks);

  const settingsConfig =
    focusState.activeWindow === "setupWizardModal"
      ? focusState.wizardConfig
      : null;
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (wizardTask && wizardPrompt) {
      const current =
        wizardPrompt.getCurrentPrompt() as SetupWizardPrompt | null;
    ...
    (input, key) => {
      ...
      if (key.downArrow) {
        setFocusedIndex((prev) =>
          Math.min(interactiveItems.length - 1, prev + 1),
        );
        return;
      }
    ...
    >
      ...
      >
                ...
                <Box key={bi} flexDirection="column">
                  {block.items.map((item, ii) => {
                    ...
                    const isFocused =
                      isActive &&
                      item.type === "link" &&
                      focusedIndex === myIndex;
                    return (
                      <Box key={ii} flexDirection="row">
                        <Text
                          color={
                            isFocused ? theme.text.active : theme.text.muted
                          }
                        >
                          {isFocused ? "☛ " : "  "}
                        </Text>
        ...
        <Box marginTop={1} flexDirection="row" flexWrap="wrap">
          ...
          <Hint label="Disable the service" shortcut="D" />
        </Box>
      </Box>
    </Box>
  );
