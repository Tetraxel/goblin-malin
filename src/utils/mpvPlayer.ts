...
export class MpvPlayer extends EventEmitter {
    ...
    private async _start(): Promise<void> {
        ...
        this.mpvProcess = spawn(
            mpvBin,
            ["--idle=yes", "--no-video", "--no-terminal", "--really-quiet", `--input-ipc-server=${this.mpvIpcArg}`],
            ...
            new Promise((resolve, reject) => {
                ...
                const onError = (err: NodeJS.ErrnoException) => {
                    ...
                    if (++attempt >= maxAttempts) {
                        reject(new Error(`mpv IPC connect failed after ${maxAttempts} attempts: ${err.message}`));
                    } else {
    ...
    private handleMessage(msg: Record<string, unknown>) {
        ...
        if (msg.event === "end-file") {
            ...
            if (reason === "error")
                this.emit("error", new Error(`mpv playback error: ${String(msg.file_error ?? "unknown")}`));
        }
    }
